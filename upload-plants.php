    ...
    if (!empty($file_name)) {
      if (($fileextension !== "jpg") && ($fileextension !== "jpeg") && ($fileextension !== "png") && ($fileextension !== "bmp") && ($fileextension !== "gif")) {
        ...
        if (move_uploaded_file($tmp_file_name, $path.$file_name) && $description) {
          $sendToDb = "insert into plant_pane(code,title,author,description,pic_upload,date) values('$postCode','$title','$author','$description','$file_name','$date') ";
          $query = mysqli_query($con, $sendToDb);
          if ($query) {
            echo "<script>alert('uploaded successfully')</script>";
            echo "<script>window.open('plantpost.php','_self')</script>";
          }else {
...
if(isset($_POST['new_reply'])){
  ...
  if(isset($rdescription)){
    mysqli_query($con, "insert into plantreply(par_code,description,date,author) values('$rpostCode','$rdescription','$rdate','$rauthor')");
  }
  header("Location: ");
}
