...
<html lang="en" dir="ltr">
  <head>
    ...
    <title>Admin Plants Page | DigitalFarmKenya</title>
  </head>
    ...
    <section class="section">
      <div class="br-modal" style="text-align:left; margin:5px; margin-bottom: 20px;">
        <div class="breadcramps">
          ...
          <a href="adm-plt.php" class="active">Plant posts</a>
        </div>
      </div>
      ...
      <div class="fetch">
        ...
        <div class="fetch-table">
          <?php
          include('../dbcon.php');
            $result = mysqli_query($con, "select * from plant_pane order by id desc") or die (mysqli_error());
          ?>
...
include('../dbcon.php');
  ...
  if(isset($_POST['user-delete'])){
    ...
    $check = mysqli_query($con, "select * from plant_pane where id='$key'");
    if (mysqli_num_rows($check)>0) {
      $delete=mysqli_query($con, "DELETE FROM plant_pane WHERE id='$key'");
      if($delete){
        echo "<script>alert('$name (s) post deleted successfully'); window.open('adm-plt.php','_self')</script>";
      }else{
        echo "<script>alert('deletion Failed'); window.open('adm-plt.php','_self')</script>";
      }
    }
  }
?>
