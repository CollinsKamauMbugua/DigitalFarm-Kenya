...
function get_total(){
  include('dbcon.php');
  $result = mysqli_query($con, "select * from plant_pane ORDER BY date DESC");
...
function get_comments(){
  ...
  foreach ($result as $item) {
    ...
    echo '<div class="comment" id="'.$par_code.'">'
            ...
            .'<img width="40%" height="70px" src="uploads/'.$item["pic_upload"].'"/><br>'
            .'<a class="link-reply" id="reply" name="'.$par_code.'">Reply</a>';

        //for pulling out any children that exist for this comment
        $child_result = mysqli_query($con, "select * from plantreply WHERE par_code = '$par_code' ORDER BY date DESC");
